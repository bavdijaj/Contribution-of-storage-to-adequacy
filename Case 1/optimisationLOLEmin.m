function [ENS,LOL,load,P_value,P_stor_value,E_stor_value,PNS,year_MC] = optimisationLOLEmin( ...
    ccgt_available_power,wind_available_power,TS_load,stor_cap,stor_ener,Nt,eta_stor)
...
% the storage when the objective function is to minimise the LOLE.
%
% Input:
...
yalmip('clear');
%Nt = 4*24; %just test if 24 hours works faster
ones_Nt = ones(1,Nt);
...
tic 
%By putting >=, there is no certainty that LS is actually equal to the real
%load shedding; nevertheless, in this optimisation, as the goal is to
%minimise LOLE, we dot not actually care about the value of the load
%shedding; of course it needs to be a load sheeding but we are only only 
%interested to see if it happens
%F = implies(load-sum(P,2)-P_stor_prod+P_stor_cons <=0, LS_bin==0);
%FF = implies(load-sum(P,2)-P_stor_prod+P_stor_cons <=0, LS==0);
%G = implies(load-sum(P,2)-P_stor_prod+P_stor_cons >=0, LS_bin==1);
%GG = implies(load-sum(P,2)-P_stor_prod+P_stor_cons >=0, LS==load-sum(P,2)-P_stor_prod+P_stor_cons);
%Constraints = [Constraints,F];
%Constraints = [Constraints, FF,GG]
%Constraints = [Constraints, LS>=0];
Constraints = [Constraints, LS >= load-sum(P,2)-P_stor_prod+P_stor_cons];
toc
%Load shedding constraints
%   Load shedding happens when production + storage is not sufficient
tic
%Constraints = [Constraints, LS_bin == ge(LS,0.01)];
toc
%   Load shedding must be higher than 0, and of course lower than the load
%   (if this load shedding happens)
tic
Constraints = [Constraints, LS >=0, LS <= load.*LS_bin];
toc

% Objective function (minimisation of cost)
tic
Objective = ones_Nt*LS_bin + ones_Nt*LS*1e-8;
toc

% Set some options for YALMIP and solver/ mipgap 1% to speed up simulations
options = sdpsettings('verbose',0,'solver','cplex','cplex.timelimit',240,'showprogress',1,'cplex.mip.tolerances.mipgap',1e-10);
...
PNS = value(load-sum(P,2)-P_stor_prod+P_stor_cons);
ENS = max(PNS,0);
LOL = LS_bin;
end